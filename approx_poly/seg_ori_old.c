...
char *argv[];
{
 ...
 arg_init(xargc,xargv,"  Calcul de l'orientation de fichier Segment (DZV)" ,
                                "15/11/93 (inrimage version 4.3)",
                                " G. Giraudon ") ;
 
      arg_get("%S","Nom de l'image (inrimage) initiale",&inname) ;
      arg_get("%S","Nom du fichier des segments (entree - DZV)",&innam1) ;
...
debug_ = arg_get("-D 0","option de debugging inrimage (facultative) sans argument");
	...
	pr_fic("stderr",innam1) ;	
	fd = op_fic_base(innam1,&TYP,&nomb,&dzv_fic,&dzv_elt,&dzv_mai) ;
...
orient(nf,buff,lfmt,mail_o,mail_e,ta_mai,seuil,bavre,identif,ntest,fen) ;
			...
			ra_fic(fd,numero,0)  ;
			wr_elt(fd,numero,nb_mai,bavre,identif)  ;
			wr_mai(fd,mail_o)  ;
			wr_mai(fd,mail_e)  ;
			}
		ntest = 0 ;
...
/*	while( re_elt(fd,&numero,&nb_mai,bavre,identif) )
		{
		de_bug("erreur sur la lecture ") ;
		if (nb_mai != 2 )
			{
