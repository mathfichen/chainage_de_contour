...
{ 
  ...
  for (i=0 ; i < nomcha ; i++ )
	...
	if (idebug == 7) 
	       {
               printf("********** fusion : chaine %d (%d maillons ) *******\n",i+1,abs(kbruit[i]));
	       }
	x = kbuf[i];
	if ( x != NIL )
	       {

/* GG : Version du 17/04/89 : on traite le cas special d'une chaine 
 de longueur 1 qui a deux liens en forcant a avoir un fils et un aieul 
 de maniere a ne pas eliminer : utile pour des traitements multi echelle 
 Ces lignes sont la difference avec la version afus_V0.c               */
	       if (abs(kbruit[i]) ==1)
			{ parents = laieul_(x,&nb_parents);
			  if (nb_parents ==2)
				{ fils_(x,parents[1]);
				  ssaieul_(x,parents[1]);
				  invers_orient(kbuf,
					      ((x->succ)->succ)->u.e.famille,
					     i+1,idebug);
				}
			} 
